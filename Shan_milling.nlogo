...
;         [robot_procedure]
  ...
  if stable_phase = 1 and stability_val > 1000
  [
    set end_flag  1
  ]
...
to do-plots
  set-current-plot "Sensor and Actuation of Robot 0"
  set-current-plot-pen "Sensor Output"
  plot [s] of robot 0

  set-current-plot-pen "Omega"
  plot [omega / 30] of robot 0
...
to score_procedure
 ...
 ask robots
  ...
  if end_flag < num-of-runs  and ticks > 0 and stable_phase = 1 and stability_val > 1000
  [
...
to robot_procedure
  ...
  do_sensing ; does the sensing to detect whatever the robot is set to detec
    ...
    ifelse length fov-list-robots > 0
    [
      set s 1
    ]
    [
    ...
    if ticks > (t_s + wait_time) and wait_flag = 0
  [
   ifelse s = 1
    [set omega_val -1]
    [set omega_val 1]
...
;  set omega (-2 + 2(s / 1 + exp(- k * (ticks - t_s)
...
to measure_results
  ...
  [ ;find_resultant_angle
    let V sqrt ((item 0 true_velocity * item 0 true_velocity) + (item 1 true_velocity * item 1 true_velocity))
    set V-sum V-sum + V
   ]
   ...
   ifelse current_radius > old_radius;if the moving average window increases, it is growing
     [
  ...
  [
   ...
   if ticks mod 1000 = 0
     [
      set old_circliness_avg mean circliness_list
...
to place_robots; defines region and/or orientation of where the robots should start
  if robot_setup = "Random"
   [
      ...
      let tycor one-of (range (-30) (30) 0.01)
  ...
  if robot_setup = "Circle - Center"
   [
    let sr_patches patches with [(distancexy 0 0 < (sqrt(number-of-robots) * ([size] of robot (0)) * (3) ) + 1) and pxcor != 0 and pycor != 0]
    move-to one-of sr_patches with [(not any? other turtles in-radius ([size] of robot (0)))]
  ...
  if robot_setup = "Circle - Center - Facing Out"
   [
    let sr_patches patches with [(distancexy 0 0 < (sqrt(number-of-robots) * ([size] of robot (0)) * (1) ) + 1) and pxcor != 0 and pycor != 0]
    move-to one-of sr_patches with [(not any? other turtles in-radius ([size] of robot (0)))]
     setxy (xcor + 0.01) (ycor + 0.01)
          ...
          if (abs(real-bearing) < ((vision-cc / 2))) and (distance-nowrap (robot (i )) < (vision-dd * 1));
           [
             set fov-list-robots fput (robot (i)) fov-list-robots
              if [sanctuary_detected_flag] of robot (i) = 1
              [
               set fov-list-green-robots fput (robot (i)) fov-list-green-robots
              ]
           ]
        ]
     set i (i + 1)
...
1622
1513
Sensor and Actuation of Robot 0
NIL
NIL
0.0
10.0
...
"" ""
PENS
"Sensor Output" 1.0 0 -2674135 true "" ""
"Omega" 1.0 0 -13345367 true "" ""
...
<experiments>
  <experiment name="Circliness_parameter_sweep" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="250001"/>
    <exitCondition>stable_phase = 1</exitCondition>
    <metric>phase</metric>
    <enumeratedValueSet variable="seed-no">
      <value value="1"/>
    </enumeratedValueSet>
    <steppedValueSet variable="number-of-robots" first="3" step="1" last="20"/>
    <steppedValueSet variable="wait_time" first="3" step="3" last="30"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
